...
/*
  ADAM is a SIMD accelerated cryptographically secure pseudorandom number generator (CSPRNG), 
  inspired by ISAAC64. It is a digital implementation of the 4 dimensional chaos system 
  described in the paper "A New 4D Chaotic System with Coexisting Hidden Chaotic Attractors" 
  that can generate a configurable amount (default 256) of 64-bit integers or doubles, with
  enough flexibility to meet different precision needs
*/

alignas(2048) static u64 frt[FRUIT_SIZE] = {0};

static u64 a, b, x, y, z, w;
...
u8 CSPRNG::generate(u64 sd) {
  /*  
    Start by generating a and b values for the system parameters: 2 positive real numbers

    They are obtained through arithmetic performed on 8 64-bit IV's that correspond to
    the verse "Be fruitful and multiply, and replenish the earth (Genesis 1:28)", mixed
    with either a provided 64-bit seed or one generated from Intel's TRNG functions

    Numerical constants are derived from ISAAC's rngstep macro
  */ 

 alignas(512) static u64 IV[8] = {
  0x4265206672756974UL, 
  0x66756C20616E6420UL, 
  0x6D756C7469706C79UL, 
  0x2C20616E64207265UL, 
  0x706C656E69736820UL, 
  0x7468652065617274UL, 
  0x68202847656E6573UL, 
  0x697320313A323829UL 
 };
  ...
  ISAAC_MIX(IV[6], IV[7], IV[4], IV[5], IV[2], IV[3], IV[0], IV[1]) 
  ISAAC_MIX(IV[1], IV[0], IV[3], IV[2], IV[5], IV[4], IV[7], IV[6]) 
  ISAAC_MIX(IV[7], IV[6], IV[5], IV[4], IV[3], IV[2], IV[1], IV[0]) 

  a = ~(IV[0]^(sd<<21)) + sd^(IV[2]>>5) + IV[4]^(sd<<12) + sd^(IV[6]>>33);
  
  b = ~(IV[7]^(sd<<21)) + sd^(IV[5]>>5) + IV[3]^(sd<<12) + sd^(IV[1]>>33);
...
//   ISAAC_MIX(frt[4],frt[12],frt[48],frt[56], seed, seed, seed, seed)
//   ISAAC_MIX(frt[132],frt[140],frt[176],frt[184],frt[68],frt[76],frt[112],frt[120])
//   ISAAC_MIX(seed, seed, seed, seed,frt[196],frt[204],frt[240],frt[248])
//   ISAAC_MIX(frt[132],frt[140],frt[176],frt[184],frt[68],frt[76],frt[112],frt[120])
...
//     frt[7+rounds] ^= frt[135-rounds] ^= frt[15+rounds]      ^= frt[143-rounds] ^= frt[51+rounds] 
//     ^= frt[179-rounds] ^= frt[59+rounds] ^= frt[187-rounds] ^= frt[71+rounds]  ^= frt[199-rounds] 
//     ^= frt[79+rounds] ^= frt[207-rounds] ^= frt[115+rounds] ^= frt[243-rounds] ^= frt[123+rounds] 
//     ^= frt[251-rounds] += seed;
//   } while (rounds--);
...
//     ISAAC_MIX(frt[4],frt[12],frt[48],frt[56],frt[68],frt[76],frt[112],frt[120])
//     ISAAC_MIX(frt[132],frt[140],frt[176],frt[184],frt[196],frt[204],frt[240],frt[248])
//     ISAAC_MIX(frt[7], frt[15], frt[51], frt[59], frt[71], frt[79], frt[115], frt[123]) 
//     ISAAC_MIX(frt[135], frt[143], frt[179], frt[187], frt[199], frt[207], frt[243], frt[251])
...
// static inline void undulate(u8 undulation) {
