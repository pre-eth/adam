...
static double mod_table[BUF_SIZE] ALIGN(SIMD_LEN) = {
    ...
    6148914691236516864.0, 9218868437227405312.0, 9223372036854775808.0, 18446744073709551615.0};
...
#define LIMIT 0.5
  ...
  */
  u64 IV[8] ALIGN(SIMD_LEN) = {0x4265206672756974ULL ^ seed[2], 0x66756C20616E6420ULL ^ ~seed[2],
  ...
  i = 0;
  do {
    IV[0] += _ptr[IV[7] & 0xFF], IV[1] += _ptr[IV[6] & 0xFF], IV[2] += _ptr[IV[5] & 0xFF],
        IV[3] += _ptr[IV[4] & 0xFF], IV[4] += _ptr[IV[3] & 0xFF], IV[5] += _ptr[IV[2] & 0xFF],
    ...
    IV[0] += _ptr[IV[5] & 0xFF], IV[1] += _ptr[IV[4] & 0xFF], IV[2] += _ptr[IV[3] & 0xFF],
        IV[3] += _ptr[IV[2] & 0xFF], IV[4] += _ptr[IV[1] & 0xFF], IV[5] += _ptr[IV[0] & 0xFF],
        IV[6] += _ptr[IV[7] & 0xFF], IV[7] += _ptr[IV[6] & 0xFF];
    ...
    // << 3)], chseeds[(i << 3) + 1], chseeds[(i << 3) + 2], chseeds[(i << 3) +
    // 3], chseeds[(i << 3) + 4], chseeds[(i << 3) + 5], chseeds[(i << 3) + 6],
    // chseeds[(i << 3) + 7]);
  } while (++i < (ROUNDS >> 1));
}
#else
...
fill_the_earth:
  ...
  do {
    ...
    s1 = _mm256_setr_pd((double)(IV[0] ^ IV[4]), (double)(IV[1] ^ IV[5]), (double)(IV[2] ^ IV[6]),
                        (double)(IV[3] ^ IV[7]));
    ...
    s2 = _mm256_setr_pd((double)(IV[1] ^ IV[4]), (double)(IV[0] ^ IV[5]), (double)(IV[3] ^ IV[6]),
                        (double)(IV[2] ^ IV[7]));
...
FORCE_INLINE static void diffuse(u64 *_ptr, const u64 nonce) {
  ...
  do {
    a += _ptr[i], b += _ptr[i + 1], c += _ptr[i + 2], d += _ptr[i + 3], e += _ptr[i + 4],
        f += _ptr[i + 5], g += _ptr[i + 6], h += _ptr[i + 7];

    ISAAC_MIX(a, b, c, d, e, f, g, h);

    _ptr[i] = a, _ptr[i + 1] = b, _ptr[i + 2] = c, _ptr[i + 3] = d, _ptr[i + 4] = e,
    _ptr[i + 5] = f, _ptr[i + 6] = g, _ptr[i + 7] = h;
...
FORCE_INLINE static void apply(u64 *_ptr, double *chseeds) {
  ...
  u64 arr[8] ALIGN(64);
chaotic_iter:
  arr[0] = 1UL, arr[1] = 3UL, arr[2] = 2UL, arr[3] = 4UL, arr[4] = 5UL, arr[5] = 6UL, arr[6] = 7UL,
  arr[7] = 8UL;
...
#else
FORCE_INLINE static void apply(u64 *_ptr, double *chseeds) {
  const regd beta = SIMD_SETQPD(BETA), coefficient = SIMD_SETQPD(3.9999), one = SIMD_SETQPD(1.0);
